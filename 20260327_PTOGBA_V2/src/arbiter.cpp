    ...
    while (true) {
        ...
        if (!high_priority_queue_.empty()) {
            ...
            if (allocate_resource(request, path)) {
                ...
                // Spawn 独立线程持有资源（并发执行）
                sc_core::sc_spawn(sc_core::sc_bind(&HybridArbiter::hold_and_release_resource, this, request, path));
        ...
        if (high_priority_queue_.empty() && !normal_queue_.empty()) {
            ...
            if (allocate_resource(request, path)) {
                ...
                // V2.2: 普通队列也需要并发持有资源！
                sc_spawn(sc_core::sc_bind(&HybridArbiter::hold_and_release_resource, this, request, path));
            }
...
// V2.2: 并发持有资源方法实现
// ============================================================================
void HybridArbiter::hold_and_release_resource(BarrierRequest request, PathType path) {
    // 计算持有时间（基于组大小）
